...
<head>
	<script type="text/javascript">
		...
		function validarFormulario(evObject) {
			...
			if(formulario[8].value == null || formulario[8].value.length == 0 || /^\s*$/.test(formulario[8].value)){
				elem8.setAttribute('class','form-group has-error ');
				todoCorrecto=false;
			}else {elem8.setAttribute('class','form-group has-success'); }

			if(formulario[9].value == null || formulario[9].value.length == 0 || /^\s*$/.test(formulario[9].value)){
				elem9.setAttribute('class','form-group has-error ');
				todoCorrecto=false;
			}else {elem9.setAttribute('class','form-group has-success'); }
...
<body>
	...
	<div class="container-fluid" style="height: 500px;">
 	...
 	<br>
 		...
 		<form action="_editarDatosFinalizado.php" class="form-horizontal" role="form" method="post" name="formSolicitud">
				...
				<div class="form-group" id="elem8"> 
			    	<label class="col-sm-offset-1 col-sm-2 control-label">Dictamen</label> 
			    	<div class="col-sm-7">
			    		<textarea class="form-control" rows="3" name="dict"><?php echo($dict);?></textarea> 
			    	</div>
				</div>

				<div class="form-group" id="elem9"> 
			    	<label class="col-sm-offset-1 col-sm-2 control-label">Coment</label> 
			    	<div class="col-sm-7">
			    		<textarea class="form-control" rows="3" name="coment"><?php echo($coment);?></textarea> 
			    	</div>
				</div>
				<div class="form-group" id="elem10"> 
			    	<label  class="col-sm-offset-1 col-sm-2 control-label">Resultado:</label> 
			    	<div class="col-sm-7">
			    		<label class="radio-inline"><input type="radio" name="res" value="Aprobado" id="rd1" <?php if($status ==5 ){echo ("checked");}?>> Aprobado </label>
						<label class="radio-inline"><input type="radio" name="res" value="Rechazado" id="rd2" <?php if($status ==4 ){echo ("checked");}?>> Rechazado </label>
			    	</div>
				</div>
